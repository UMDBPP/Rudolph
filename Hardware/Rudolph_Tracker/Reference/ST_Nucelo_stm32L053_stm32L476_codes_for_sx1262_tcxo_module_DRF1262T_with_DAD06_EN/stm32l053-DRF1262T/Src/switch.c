...
SWITCH_STATE_ST Check_switch(void)
{
	...
	uint16_t sb_val = (GPIOC->IDR&0x70)>>4;  //check the selection of Bandwidth
	uint16_t sf_val = (GPIOC->IDR&0x1c00)>>10; //check the selection of Spreading Factor
	
	if(HAL_GPIO_ReadPin(DORJI_SB4_GPIO_Port,DORJI_SB4_Pin))
	{
		sb_val |= 1<<3;
	}
	select_mode.band_width = (~sb_val)&0x0f; //Reverse the value of low 4 bits
	select_mode.spread_factor = switch_sf_val[(~sf_val)&0x07];//Reverse the value of low 3 bits
	
	/*Check the selection of T/R*/
